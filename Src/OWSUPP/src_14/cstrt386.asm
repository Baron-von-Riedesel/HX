...
;*               modified to run with HX DOS extender. 
;*               this is the startup code used by HX PE binaries.
;*               Search for string 'HX' to find all modified places
;*
;*****************************************************************************
...
;--- the HX PE loader will alloc the stack in its own memory block.
;--- that means bottom of stack is *not* at the end of _BSS (_end)

;        mov     edx,offset DGROUP:_end
		xor		edx, edx
		mov		ax,4b82h
        int		21h
		mov		edx, [eax+2Ch]			; get the stack lower limit

;--- end of HX modifications
...
;        mov     eax,offset DGROUP:_end  ; cmd buffer pointed at by EAX
		...
		mov		ax,4b82h
        int		21h
		mov		eax, [eax+2Ch]			; get the stack lower limit
...
__GETDSStart_ label byte
        ...
        ret                             ; return
;--- HX PE loader will protect code sections! store DS in a writeable section
_DATA segment
